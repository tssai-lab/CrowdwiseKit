...
def main(args):
    ...
    for step in range(args.epoch):
        ...
        torch.save(maml, 'opre_net3.pth')

def finetune():
    device = torch.device('cuda')
    model = torch.load('opre_net1.pth').to(device)
    ...
    for _ in range(1000//args.task_num):
        ...
        for x_spt_one, y_spt_one, x_qry_one, y_qry_one in zip(x_spt, y_spt, x_qry, y_qry):
            feature = model.finetunning(x_spt_one, y_spt_one, x_qry_one, y_qry_one)
    ...
    #     truth.append([id,x+1])
    #     id = id +1
    # f=open('data/omniglot_truth1.glod','w')
    ...
    #     features.append(y)
    #     id = id +1
    # fp=open('data/omniglot_feature1.attr','w')
