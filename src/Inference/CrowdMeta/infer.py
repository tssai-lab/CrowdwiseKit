...
def function(var, xs, ls, zs):
    ...
    for i in range(1,xs.shape[0]+1):
        for j in range(ls.shape[1]):
            ...
            difficult = (1 + np.exp(-diff[i-1])) ** -1

            num += (np.log(prob + epsilon) + np.log(difficult + epsilon) + np.log(show(ls[i][j], zs[i], prob) + epsilon))
    ...
    for i in range(1,xs.shape[0]+1):
        ...
        for j in range(0,ls.shape[1]):
            ...
            for k in range(0, label_num):
                p[k] += np.log(prob + epsilon) + np.log(difficult + epsilon) + np.log(show(ls[i][j], k+1, prob) + epsilon)
...
def EM(xs, ls, truth, max_iter=8, tol=1e-2):
...
if __name__ == '__main__':
    ...
    dist = []
    with open(distrifile) as f:
        for line in f:
            temp = line.strip('\n').split(' ')
            dist.append(list(map(float, temp)))

    diff = np.zeros(vec.shape[0])
    diff[0]=0
    for m in range(1,lss.shape[1]):
         diff[m] = random.gauss(dist[m-1][0],dist[m-1][1])
